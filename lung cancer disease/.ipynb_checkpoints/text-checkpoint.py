...
class App():
    ...
    # refresh tampilan chart
        ...
        self.canvas2.draw()
        # self.buttontab2 = ttk.Button(
        #     self.frame2, text='Update Chart', style='Accent.TButton', command=self.grafik)
        # self.buttontab2.grid(row=0, column=0)
    ...
    def insert_row(self):
        ...
        # self.order = {'High': 0, 'Medium': 1, 'Low': 2}
        # df_sorted = self.df.sort_values(
        #     by='Level', key=lambda x: x.map(self.order))
        # print(df_sorted)

        # df = pd.read_excel("data1 copy.xlsx")
        # priority_queue = queue.PriorityQueue()
        # priority_queue = self.order
    ...
    # Insert row into Excel sheet
        ...
        row_values = [self.tanggal, self.name,
                      ...
                      self.chestpain_combobox.get(), self.coughingofblood_combobox.get(), self.fatigue_combobox.get(), self.weightloss_combobox.get(), self.shortnessofbreath_combobox.get(), self.wheezing_combobox.get(), self.swallowingdiffi_combobox.get(), self.clubbing_combobox.get(), self.frequentcold_combobox.get(), self.drycough_combobox.get(), self.snoring_combobox.get(), self.hasiltrain]
    ...
    def grafik(self):
        ...
        self.barlist = self.ax1.bar(
            self.total_categori.keys(), self.total_categori)
        # self.barlist[0].set_color('#9CA777')
        # self.barlist[1].set_color('#64C2A6')
        # self.barlist[2].set_color('#AADEA7')
